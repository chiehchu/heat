...
class DockerContainer(resource.Resource):
    ...
    properties_schema = {
        DOCKER_ENDPOINT: properties.Schema(
            ...
            _('Kubernetes daemon endpoint (by default the local docker daemon '
              'will be used).'),
            default=None
        ),
...
def resource_mapping():
    return {
        'GoogleInc::Kubernetes::Container': DockerContainer,
    }
...
def available_resource_mapping():
    if KUBERNETES_INSTALLED:
        ...
        LOG.warn(_("Kubernetes plug-in loaded, but docker lib not installed."))
        return {}
